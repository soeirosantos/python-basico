...
(Repare que estamos adotando medidas alternativas temporariamente caso o valor informado seja inválido. Já já melhoremos isso.)
...
class Funcionario(object):
    ...
    def data_de_admissao(self, salario):
        if self.eh_data_valida(salario):
            self._data_de_admissao = str_to_date(salario)
        else:
            self._data_de_admissao = date.today()
